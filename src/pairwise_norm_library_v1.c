/**************************************************************************
 * Pairwise normalization method (version 1).
 *
 * This is a direct port of Eric Welsh's IRON pairwise normalization
 * approach to R. Minimal changes are implemented in this version - only
 * those needed to successfully interoperate with R. This is generally
 * involving memory allocation and data types.
 **************************************************************************/
                                  ...
                                  int num_pairs)
{
  ...
  for (i = 0; i < num_pairs; i++)
  {
    ...
    if (isnan(pair_ptr->fit_log_adjust))
      Rprintf("NAN_LOGADJUST %f\n", x);
    if (isnan(avg_adjust))
      Rprintf("NAN_AVGADJUST %f\n", x);
#endif
  }
}
                                     ...
                                     int fit_both_x_y_flag)
{
  ...
  xy_pairs = R_Calloc(num_pairs_train, struct xy_pair);
  ...
  xy_pairs2 = R_Calloc(num_pairs_train,struct xy_pair);
...
int fill_normalization_scales1(const char *filestem,
                               double *signals1,
                               double *signals2,
                               double *signals2_scales, //return result
                               int *mask_array, // flag same length as signals1/2/scales
                               int num_spots,
                               double rank_frac_cutoff,
                               double rank_frac_cutoff2,
                               int condense_training_flag,
                               int iron_global_scaling_normalization,
                               int iron_fit_both_x_y,
                               int iron_weight_exponent,
                               int iron_untilt_normalization,
                               double iron_fit_window_frac,
                               double *return_training_frac, //return result
                               double *return_rmsd) // return result
{
  ...
  int    global_scaling_flag      = iron_global_scaling_normalization;
  int    fit_both_x_y_flag        = iron_fit_both_x_y;
  double weight_exponent          = iron_weight_exponent;
  ...
  if (i == num_spots || num_both_not_weak == 0)
  {
    ...
    *return_rmsd = 0.0;
    ...
    if (global_scaling_flag)
      REprintf("GlobalScale:\t%s\t%f\t%f\t%d\t%d\t%d\t%d\t%f\n",
                      filestem,
                      1.0, 0.0,
                      num_not_weak,
                      num_both_not_weak,
                      ...
                      1.0);
    else if (iron_untilt_normalization)
      REprintf("GlobalFitLine:\t%s\t%f\t%f\t%f\t%d\t%d\t%d\n",
                      filestem,
                      1.0, 0.0, 0.0,
  ...
  signal_pairs = R_Calloc(num_spots, struct signal_pair);
  tmp_ptrs1 = R_Calloc(num_spots, struct signal_pair *);
  tmp_ptrs2 = R_Calloc(num_spots, struct signal_pair *);
  tmp_ptrs3 = R_Calloc(num_spots, struct signal_pair *);
  tmp_ptrs4 = R_Calloc(num_spots, struct signal_pair *);
  ...
  eqn_windows = R_Calloc(num_filtered, struct eqn_window);
  ...
  num_eqns = fill_geometric_eqn_windows(eqn_windows, filt2,
                                        num_filtered, iron_fit_window_frac,
                                        2, weight_exponent);
  smooth_geometric_fits(eqn_windows, num_eqns, filt2, num_filtered);
#endif
  ...
  if (!debug_file)
    REprintf("ERROR -- Can't open irank_set.txt\n");
...
#if DEBUG_PRINT
  REprintf("SimilarityMetrics:\tTrain\t%f\tRMSD\t%f\n",
...
SEXP R_fill_normalization_scales_(
    SEXP filestem,
    SEXP signals1,
    SEXP signals2,
    SEXP mask_array,
    SEXP num_spots,
    SEXP rank_frac_cutoff,
    SEXP rank_frac_cutoff2,
    SEXP condense_training_flag,
    SEXP iron_global_scaling_normalization,
    SEXP iron_fit_both_x_y,
    SEXP iron_weight_exponent,
    SEXP iron_untilt_normalization,
    SEXP iron_fit_window_frac
)
{
  const char *filestem_ = CHAR(STRING_ELT(filestem,0));
  double *signals1_ = REAL(signals1);
  double *signals2_ = REAL(signals2);

  int *mask_array_ = LOGICAL(mask_array);
  int num_spots_ = INTEGER(num_spots)[0];
  double rank_frac_cutoff_ = REAL(rank_frac_cutoff)[0];
  double rank_frac_cutoff2_ = REAL(rank_frac_cutoff2)[0];
  int condense_training_flag_ = INTEGER(condense_training_flag)[0];
  int iron_global_scaling_normalization_ = INTEGER(iron_global_scaling_normalization)[0];
  int iron_fit_both_x_y_ = INTEGER(iron_fit_both_x_y)[0];
  int iron_weight_exponent_ = INTEGER(iron_weight_exponent)[0];
  int iron_untilt_normalization_ = INTEGER(iron_untilt_normalization)[0];
  double iron_fit_window_frac_ = REAL(iron_fit_window_frac)[0];

  /* Three variables: signals2_scales, return_training_frac and return_rmsd may
   * be needed in R (eventually) so for return values we protect them.
   */
  double return_training_frac;
  double return_rmsd;
  SEXP ans = PROTECT(Rf_allocVector(REALSXP, num_spots_));
  double *signals2_scales_ = REAL(ans);

  int errcode;

  errcode = fill_normalization_scales1(
    filestem_, signals1_, signals2_, signals2_scales_,
    mask_array_,num_spots_,rank_frac_cutoff_,
    rank_frac_cutoff2_,condense_training_flag_,
    iron_global_scaling_normalization_,
    iron_fit_both_x_y_, iron_weight_exponent_,
    iron_untilt_normalization_, iron_fit_window_frac_,
  &return_training_frac, &return_rmsd
  );
